...
void freeChunk(Chunk *chunk) {
  FREE_ARRAY(uint8_t, chunk->code,
             chunk->capacity); // Frees the opcode dynamic array
  FREE_ARRAY(int, chunk->lines,
             chunk->capacity);       // Frees the lines dynamic array
...
void writeChunk(Chunk *chunk, uint8_t byte, int line) {
  ...
  if (chunk->capacity < chunk->count + 1) {
    ...
    chunk->capacity = GROW_CAPACITY(
        oldCapacity); //  Grow capacity by a factor with the old capacity
  ...
  chunk->code[chunk->count] = byte; // Add byte to the count since it will be +1
                                    // from the last entry position
