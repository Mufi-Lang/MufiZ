...
fn det_impl(_: i32, args: [*]Value) Value {
    ...
    if (det_val == null) {
        return stdlib_core.stdlib_error("Matrix must be square for determinant calculation", .{});
    }
...
fn inv_impl(_: i32, args: [*]Value) Value {
    ...
    if (result == null) {
        return stdlib_core.stdlib_error("Matrix is singular or not square", .{});
    }
    return Value.init_obj(@ptrCast(result.?));
}
...
fn matrix_get_impl(_: i32, args: [*]Value) Value {
    ...
    if (row >= matrix.rows or col >= matrix.cols) {
        return stdlib_core.stdlib_error("Matrix index out of bounds", .{});
    }
...
fn matrix_set_impl(_: i32, args: [*]Value) Value {
    ...
    if (args[3].is_double()) {
        ...
        return stdlib_core.stdlib_error("Value must be numeric", .{});
    }
...
fn horzcat_impl(_: i32, args: [*]Value) Value {
    ...
    if (result == null) {
        return stdlib_core.stdlib_error("Matrices must have the same number of rows for horizontal concatenation", .{});
    }
...
fn vertcat_impl(_: i32, args: [*]Value) Value {
    ...
    if (result == null) {
        return stdlib_core.stdlib_error("Matrices must have the same number of columns for vertical concatenation", .{});
    }
...
fn matrix_create_impl(_: i32, args: [*]Value) Value {
    ...
    if (vector.count != rows * cols) {
        return stdlib_core.stdlib_error("Vector size does not match matrix dimensions", .{});
    }
...
fn reshape_impl(_: i32, args: [*]Value) Value {
    ...
    if (matrix.rows * matrix.cols != new_rows * new_cols) {
        return stdlib_core.stdlib_error("Number of elements must remain the same when reshaping", .{});
    }
