...
fn assert_impl(argc: i32, args: [*]Value) Value {
    if (argc == 2) {
        ...
        if (!valuesEqual(args[0], args[1])) {
            ...
            return stdlib_core.stdlib_error("Assertion failed: Expected '{}', got '{}'", .{ expected_str, actual_str });
        }
    } else {
        ...
        if (!is_truthy) {
            ...
            return stdlib_core.stdlib_error("Assertion failed: Expected truthy value, got {s}", .{value_type});
        }
    }
...
fn format_impl(argc: i32, args: [*]Value) Value {
    ...
    const buffer = allocator.alloc(u8, template.len * 2) catch {
        return stdlib_core.stdlib_error("Failed to allocate buffer for format", .{});
    };
