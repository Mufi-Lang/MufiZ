...
pub fn what_is(argc: c_int, args: [*c]Value) Value {
...
// pub fn import(argc: c_int, args: [*c]Value)  Value {
...
const net_funs = if (enable_net) struct {
    pub fn get(argc: c_int, args: [*c]Value) Value {
        // expects `(url, method)`
    ...
    pub fn post(argc: c_int, args: [*c]Value) Value {
        // expects `(url, method)`
    ...
    pub fn put(argc: c_int, args: [*c]Value) Value {
        // expects `(url, method)`
        if (argc < 3) return stdlib_error("get() expects at least 2 arguments!", .{ .argn = argc });
        const url = args[0].as_zstring();
        const data = args[1].as_zstring();
    ...
    pub fn delete(argc: c_int, args: [*c]Value) Value {
        // expects `(url, method)`
        if (argc < 2) return stdlib_error("get() expects at least 2 arguments!", .{ .argn = argc });
        const url = args[0].as_zstring();
        const method: u8 = @intCast(args[1].as_int());
    ...
    pub fn open(argc: c_int, args: [*c]Value) Value {
