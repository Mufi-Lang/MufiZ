...
pub fn sliceArray(arg_array: [*c]ObjArray, arg_start: c_int, arg_end: c_int) [*c]ObjArray {
    ...
    var end = arg_end;
    _ = &end;
    var sliced: [*c]ObjArray = newArrayWithCap(end - start, false);
    _ = &sliced;
    {
        var i: c_int = start;
        _ = &i;
...
pub fn swapRow(arg_matrix: [*c]ObjMatrix, arg_row1: c_int, arg_row2: c_int) void {
    ...
    var row2 = arg_row2;
    _ = &row2;
    if (((((matrix != null) and (row1 >= 0)) and (row1 < matrix.*.rows)) and (row2 >= 0)) and (row2 < matrix.*.rows)) {
        {
            var col: c_int = 0;
            _ = &col;
...
pub fn newMatrix(arg_rows: c_int, arg_cols: c_int) [*c]ObjMatrix {
    ...
    matrix.*.data = newArrayWithCap(matrix.*.len, false);
    {
        var i: c_int = 0;
        _ = &i;
        while (i < matrix.*.len) : (i += 1) {
...
pub fn setRow(arg_matrix: [*c]ObjMatrix, arg_row: c_int, arg_values: [*c]ObjArray) void {
    ...
    _ = &values;
    if ((((matrix != null) and (values != null)) and (row >= 0)) and (row < matrix.*.rows)) {
        {
            var col: c_int = 0;
            _ = &col;
            while (col < matrix.*.cols) : (col += 1) {
...
pub fn setCol(arg_matrix: [*c]ObjMatrix, arg_col: c_int, arg_values: [*c]ObjArray) void {
    ...
    var values = arg_values;
    _ = &values;
    if ((((matrix != null) and (values != null)) and (col >= 0)) and (col < matrix.*.cols)) {
        {
            var row: c_int = 0;
            _ = &row;
...
pub fn setMatrix(arg_matrix: [*c]ObjMatrix, arg_row: c_int, arg_col: c_int, arg_value: Value) void {
    ...
    var value = arg_value;
    _ = &value;
    if (((((matrix != null) and (row >= 0)) and (row < matrix.*.rows)) and (col >= 0)) and (col < matrix.*.cols)) {
...
pub fn sortFloatVector(arg_vector: [*c]FloatVector) void {
    ...
    qsort(@as(?*anyopaque, @ptrCast(vector.*.data)), @intCast(vector.*.count), @sizeOf(f64), &compare_double);
}
