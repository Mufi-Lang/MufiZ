...
pub fn errorAt(arg_token: [*c]Token, arg_message: [*c]const u8) callconv(.C) void {
    ...
    _ = fprintf(stderr, "[line %d] Error", token.*.line);
    if (token.*.type == @as(c_uint, @bitCast(TOKEN_EOF))) {
        _ = fprintf(stderr, " at end");
    } else if (token.*.type == @as(c_uint, @bitCast(TOKEN_ERROR))) {} else {
        _ = fprintf(stderr, " at '%.*s'", token.*.length, token.*.start);
    }
    _ = fprintf(stderr, ": %s\n", message);
...
pub fn advance() callconv(.C) void {
    ...
    while (true) {
        ...
        if (parser.current.type != @as(c_uint, @bitCast(TOKEN_ERROR))) break;
        errorAtCurrent(parser.current.start);
    }
}
pub fn consume(arg_type: enum_TokenType, arg_message: [*c]const u8) callconv(.C) void {
    var @"type" = arg_type;
    _ = &@"type";
    var message = arg_message;
    _ = &message;
...
pub fn check(arg_type: enum_TokenType) callconv(.C) bool {
    var @"type" = arg_type;
    _ = &@"type";
    return parser.current.type == @"type";
}
pub fn match(arg_type: enum_TokenType) callconv(.C) bool {
    var @"type" = arg_type;
    _ = &@"type";
...
pub fn emitLoop(arg_loopStart: c_int) callconv(.C) void {
    ...
    _ = &loopStart;
    emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_LOOP)))));
...
pub fn makeConstant(arg_value: Value) callconv(.C) u8 {
    var value = arg_value;
    _ = &value;
    var constant: c_int = addConstant(currentChunk(), value);
    _ = &constant;
...
pub fn statement() callconv(.C) void {
    if (match(@as(c_uint, @bitCast(TOKEN_PRINT)))) {
        printStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_FOR)))) {
        forStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_EACH)))) {
        eachStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_IF)))) {
        ifStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_RETURN)))) {
        returnStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_WHILE)))) {
        whileStatement();
    } else if (match(@as(c_uint, @bitCast(TOKEN_LEFT_BRACE)))) {
        beginScope();
        block();
        endScope();
    } else {
...
pub fn declaration() callconv(.C) void {
    if (match(@as(c_uint, @bitCast(TOKEN_CLASS)))) {
        classDeclaration();
    } else if (match(@as(c_uint, @bitCast(TOKEN_FUN)))) {
        funDeclaration();
    } else if (match(@as(c_uint, @bitCast(TOKEN_VAR)))) {
        varDeclaration();
    } else {
        statement();
    }
...
pub fn getRule(arg_type: enum_TokenType) callconv(.C) [*c]ParseRule {
    var @"type" = arg_type;
    _ = &@"type";
    return &rules[@"type"];
}
pub fn parsePrecedence(arg_precedence: Precedence) callconv(.C) void {
    ...
    if ((@as(c_int, @intFromBool(canAssign)) != 0) and (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_EQUAL))))) != 0)) {
...
pub fn identifierConstant(arg_name: [*c]Token) callconv(.C) u8 {
    ...
    return makeConstant(Value{
        ...
        .as = union_unnamed_3{
            .obj = @as([*c]Obj, @ptrCast(@alignCast(copyString(name.*.start, name.*.length)))),
        },
    });
}
pub fn identifiersEqual(arg_a: [*c]Token, arg_b: [*c]Token) callconv(.C) bool {
    ...
    return memcmp(@as(?*const anyopaque, @ptrCast(a.*.start)), @as(?*const anyopaque, @ptrCast(b.*.start)), @as(c_ulong, @bitCast(@as(c_long, a.*.length)))) == @as(c_int, 0);
}
...
pub fn parseVariable(arg_errorMessage: [*c]const u8) callconv(.C) u8 {
    ...
    _ = &errorMessage;
    consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), errorMessage);
    declareVariable();
...
pub fn argumentList() callconv(.C) u8 {
    var argCount: u8 = 0;
    _ = &argCount;
    if (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)))) {
        while (true) {
            expression();
            ...
            argCount +%= 1;
            if (!match(@as(c_uint, @bitCast(TOKEN_COMMA)))) break;
        }
    }
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after arguments.");
    return argCount;
}
...
pub fn binary(arg_canAssign: bool) callconv(.C) void {
    var canAssign = arg_canAssign;
    _ = &canAssign;
    var operatorType: enum_TokenType = parser.previous.type;
    _ = &operatorType;
...
pub fn dot(arg_canAssign: bool) callconv(.C) void {
    var canAssign = arg_canAssign;
    _ = &canAssign;
    consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), "Expect property name after '.'.");
    var name: u8 = identifierConstant(&parser.previous);
    _ = &name;
    if ((@as(c_int, @intFromBool(canAssign)) != 0) and (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_EQUAL))))) != 0)) {
        expression();
        emitBytes(@as(u8, @bitCast(@as(i8, @truncate(OP_SET_PROPERTY)))), name);
    } else if (match(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)))) {
        var argCount: u8 = argumentList();
        _ = &argCount;
...
pub fn grouping(arg_canAssign: bool) callconv(.C) void {
    ...
    _ = &canAssign;
    expression();
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after expression.");
}
pub fn number(arg_canAssign: bool) callconv(.C) void {
    var canAssign = arg_canAssign;
    _ = &canAssign;
    if (parser.previous.type == @as(c_uint, @bitCast(TOKEN_INT))) {
...
pub fn string(arg_canAssign: bool) callconv(.C) void {
    ...
    emitConstant(Value{
        .type = @as(c_uint, @bitCast(VAL_OBJ)),
        .as = union_unnamed_3{
            .obj = @as([*c]Obj, @ptrCast(@alignCast(copyString(parser.previous.start + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1))))), parser.previous.length - @as(c_int, 2))))),
        },
    });
}
pub fn array(arg_canAssign: bool) callconv(.C) void {
    ...
    var argCount: u8 = 0;
    _ = &argCount;
    if (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_SQPAREN)))) {
        while (true) {
            expression();
            argCount +%= 1;
            if (@as(c_int, @bitCast(@as(c_uint, argCount))) > @as(c_int, 255)) {
    ...
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_SQPAREN)), "Expect ']' after array elements.");
...
pub fn fvector(arg_canAssign: bool) callconv(.C) void {
    ...
    var argCount: u8 = 0;
    _ = &argCount;
    if (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE)))) {
        while (true) {
            expression();
            argCount +%= 1;
            if (@as(c_int, @bitCast(@as(c_uint, argCount))) > @as(c_int, 255)) {
    ...
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE)), "Expect '}' after vector elements.");
...
pub fn namedVariable(arg_name: Token, arg_canAssign: bool) callconv(.C) void {
    ...
    if ((@as(c_int, @intFromBool(canAssign)) != 0) and (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_EQUAL))))) != 0)) {
    ...
    } else if ((((@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_PLUS_EQUAL))))) != 0) or (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_MINUS_EQUAL))))) != 0)) or (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_STAR_EQUAL))))) != 0)) or (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_SLASH_EQUAL))))) != 0)) {
        ...
        emitBytes(setOp, @as(u8, @bitCast(@as(i8, @truncate(arg)))));
    } else if (match(@as(c_uint, @bitCast(TOKEN_PLUS_PLUS)))) {
        emitBytes(getOp, @as(u8, @bitCast(@as(i8, @truncate(arg)))));
        ...
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_ADD)))));
        emitBytes(setOp, @as(u8, @bitCast(@as(i8, @truncate(arg)))));
    } else if (match(@as(c_uint, @bitCast(TOKEN_MINUS_MINUS)))) {
        emitBytes(getOp, @as(u8, @bitCast(@as(i8, @truncate(arg)))));
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_CONSTANT)))));
        emitByte(makeConstant(Value{
            .type = @as(c_uint, @bitCast(VAL_INT)),
...
pub fn index_(arg_canAssign: bool) callconv(.C) void {
    var canAssign = arg_canAssign;
    _ = &canAssign;
    if (check(@as(c_uint, @bitCast(TOKEN_LEFT_SQPAREN)))) {
        consume(@as(c_uint, @bitCast(TOKEN_LEFT_SQPAREN)), "Expect '[' after array.");
        expression();
        consume(@as(c_uint, @bitCast(TOKEN_RIGHT_SQPAREN)), "Expect ']' after index.");
        if ((@as(c_int, @intFromBool(canAssign)) != 0) and (@as(c_int, @intFromBool(match(@as(c_uint, @bitCast(TOKEN_EQUAL))))) != 0)) {
            expression();
...
pub fn syntheticToken(arg_text: [*c]const u8) callconv(.C) Token {
    ...
    var token: Token = Token{
        .type = @import("std").mem.zeroes(enum_TokenType),
        .start = text,
        .length = @as(c_int, @bitCast(@as(c_uint, @truncate(strlen(text))))),
        .line = 0,
    };
    _ = &token;
    return token;
...
pub fn super_(arg_canAssign: bool) callconv(.C) void {
    ...
    consume(@as(c_uint, @bitCast(TOKEN_DOT)), "Expect '.' after 'super'.");
    consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), "Expect superclass method name.");
    var name: u8 = identifierConstant(&parser.previous);
    _ = &name;
    namedVariable(syntheticToken("self"), @as(c_int, 0) != 0);
    if (match(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)))) {
        var argCount: u8 = argumentList();
        _ = &argCount;
...
pub fn block() callconv(.C) void {
    while (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE))) and !check(@as(c_uint, @bitCast(TOKEN_EOF)))) {
        declaration();
    }
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE)), "Expect '}' after block.");
}
pub fn function(arg_type: FunctionType) callconv(.C) void {
    ...
    beginScope();
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)), "Expect '(' after function name.");
    if (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)))) {
        while (true) {
    ...
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after parameters.");
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_BRACE)), "Expect '{' before function body.");
    block();
    ...
    emitBytes(@as(u8, @bitCast(@as(i8, @truncate(OP_CLOSURE)))), makeConstant(Value{
        .type = @as(c_uint, @bitCast(VAL_OBJ)),
        .as = union_unnamed_3{
            .obj = @as([*c]Obj, @ptrCast(@alignCast(function_1))),
        },
    }));
    {
        var i: c_int = 0;
...
pub fn method() callconv(.C) void {
    consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), "Expect method name.");
    var constant: u8 = identifierConstant(&parser.previous);
    _ = &constant;
    var @"type": FunctionType = @as(c_uint, @bitCast(TYPE_METHOD));
    _ = &@"type";
    if ((parser.previous.length == @as(c_int, 4)) and (memcmp(@as(?*const anyopaque, @ptrCast(parser.previous.start)), @as(?*const anyopaque, @ptrCast("init")), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 4))))) == @as(c_int, 0))) {
...
pub fn classDeclaration() callconv(.C) void {
    consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), "Expect class name.");
    ...
    if (match(@as(c_uint, @bitCast(TOKEN_LESS)))) {
        consume(@as(c_uint, @bitCast(TOKEN_IDENTIFIER)), "Expect superclass name.");
        variable(@as(c_int, 0) != 0);
    ...
    namedVariable(className, @as(c_int, 0) != 0);
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_BRACE)), "Expect '{' before class body.");
    while (!check(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE))) and !check(@as(c_uint, @bitCast(TOKEN_EOF)))) {
        method();
    }
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_BRACE)), "Expect '}' after class body.");
    emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_POP)))));
...
pub fn varDeclaration() callconv(.C) void {
    ...
    _ = &global;
    if (match(@as(c_uint, @bitCast(TOKEN_EQUAL)))) {
        expression();
    } else {
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_NIL)))));
    }
    consume(@as(c_uint, @bitCast(TOKEN_SEMICOLON)), "Expect ';' after variable declaration.");
    defineVariable(global);
}
pub fn expressionStatement() callconv(.C) void {
    expression();
    consume(@as(c_uint, @bitCast(TOKEN_SEMICOLON)), "Expect ';' after expression.");
    emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_POP)))));
}
pub fn forStatement() callconv(.C) void {
    beginScope();
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)), "Expect '(' after 'for'.");
    if (match(@as(c_uint, @bitCast(TOKEN_SEMICOLON)))) {} else if (match(@as(c_uint, @bitCast(TOKEN_VAR)))) {
        varDeclaration();
    } else {
        expressionStatement();
    }
    ...
    _ = &exitJump;
    if (!match(@as(c_uint, @bitCast(TOKEN_SEMICOLON)))) {
        expression();
        consume(@as(c_uint, @bitCast(TOKEN_SEMICOLON)), "Expect ';' after loop condition.");
        exitJump = emitJump(@as(u8, @bitCast(@as(i8, @truncate(OP_JUMP_IF_FALSE)))));
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_POP)))));
    }
    if (!match(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)))) {
        ...
        expression();
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_POP)))));
        consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after for clauses.");
        emitLoop(loopStart);
...
pub fn eachStatement() callconv(.C) void {
    var loopStart: c_int = currentChunk().*.count;
    _ = &loopStart;
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)), "Expect '(' after 'while'.");
    expression();
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after condition.");
    var exitJump: c_int = emitJump(@as(u8, @bitCast(@as(i8, @truncate(OP_JUMP_IF_DONE)))));
    _ = &exitJump;
    var item: Token = Token{
        .type = @as(c_uint, @bitCast(TOKEN_ITEM)),
        .start = "item",
        .length = @as(c_int, 4),
        .line = 0,
    };
...
pub fn ifStatement() callconv(.C) void {
    consume(@as(c_uint, @bitCast(TOKEN_LEFT_PAREN)), "Expect '(' after 'if'.");
    expression();
    consume(@as(c_uint, @bitCast(TOKEN_RIGHT_PAREN)), "Expect ')' after condition.");
    ...
    patchJump(thenJump);
    emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_POP)))));
    if (match(@as(c_uint, @bitCast(TOKEN_ELSE)))) {
        statement();
    }
    patchJump(elseJump);
}
pub fn printStatement() callconv(.C) void {
    expression();
    consume(@as(c_uint, @bitCast(TOKEN_SEMICOLON)), "Expect ';' after value.");
...
pub fn returnStatement() callconv(.C) void {
    ...
    if (match(@as(c_uint, @bitCast(TOKEN_SEMICOLON)))) {
        ...
        expression();
        consume(@as(c_uint, @bitCast(TOKEN_SEMICOLON)), "Expect ';' after return value.");
        emitByte(@as(u8, @bitCast(@as(i8, @truncate(OP_RETURN)))));
    }
}
...
pub fn synchronize() callconv(.C) void {
    ...
    while (parser.current.type != @as(c_uint, @bitCast(TOKEN_EOF))) {
        if (parser.previous.type == @as(c_uint, @bitCast(TOKEN_SEMICOLON))) return;
        while (true) {
